...
def get_MNIST_dataset(split: int, basepath: str=".", dimension: int = 9, numTrain: str = "00100", overlap: str = "0.00"):
    spl = str(split)
    if split < 10:
        spl = "0" + spl
    dataDir = os.path.join(basepath + SOURCE_DIR, f"ViSudo-PC_dimension::{dimension}_datasets::mnist_strategy::simple/"
    ...
    with open(os.path.join(dataDir, 'valid_puzzle_pixels.txt')) as f:
        pixels_str = f.readlines()

    with open(os.path.join(dataDir, 'valid_cell_labels.txt'), 'r') as f:
        labels_str = f.readlines()

    images_list = []
    labels_list = []
    for p, l in zip(pixels_str, labels_str):
        pp = [float(k) for k in p.split('\t')]
        ll = [int(s.split('_')[1]) for s in l.split('\t')]

        # dataset.append((pp, ll))
        images_list.append(torch.Tensor(pp).view(-1, 28, 28))
        labels_list.append(torch.Tensor(ll))  # , dtype=torch.int64))
